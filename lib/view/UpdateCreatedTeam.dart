...
class UpdateCreatedTeam extends StatefulWidget {
  var logo1,
      logo2,
      text1,
      text2,
  ...
  UpdateCreatedTeam(
      ...
      this.teamDataAll,
      this.captain_id,
      this.vice_captain_Id,
      required this.Wk_PlayersData,
      required this.BowlData_Player,
      required this.ARData_Player,
      required this.BatData_Player,
      this.total_Points,
      required this.team_Id,
      this.selected_Captain_Name,
      this.selected_Vice_Captain_Name,
      this.HighestScore,
      this.Average_Score
      }) : super(key: key);
  @override
  State<UpdateCreatedTeam> createState() => _CreateTeamState();
}

class _CreateTeamState extends State<UpdateCreatedTeam>
    with SingleTickerProviderStateMixin {
  late TabController tabController;
  List<PlayersData_> selectedPlayers =
      []; // List to keep track of selected players
...
// Define a variable to keep track of the selected players count
  ...
  void removeUnannouncedPlayersAndAdjustCredits(
      List<PlayersData_> unannouncedPlayers) {
    setState(() {
      // Remove unannounced players from selected players lists
      List<String> unannouncedPlayerIds = unannouncedPlayers
          .map((player) => player.playerId.toString())
          .toList();
      selectedPlayerWk.removeWhere((player) =>
          unannouncedPlayers.map((p) => p.playerId).contains(player.playerId));
      selectedPlayerBat.removeWhere((player) =>
          unannouncedPlayers.map((p) => p.playerId).contains(player.playerId));
      selectedPlayerAR.removeWhere((player) =>
          unannouncedPlayers.map((p) => p.playerId).contains(player.playerId));
      selectedPlayerBowl.removeWhere((player) =>
          unannouncedPlayers.map((p) => p.playerId).contains(player.playerId));
      selectedPlayers.removeWhere((player) =>
          unannouncedPlayers.map((p) => p.playerId).contains(player.playerId));
      // selectedPlayers.removeWhere((playerId) => unannouncedPlayerIds.contains(playerId));
      print("Selected Players: $selectedPlayers");
      // Update total credits
      totalCredits += unannouncedPlayers.fold(
          0, (previousValue, player) => previousValue + player.points);
    });
  }
  ...
  void getPrefrenceData() async {
    ...
    setState(() {
      _token = token!;
      Player_Data =
          fetchUpcomingMatches(widget.team1_id, widget.team2_id, _token);
      print("team_Name:::" +
          widget.text1 +
          widget.team1_id +
          ":::::" +
  ...
  void clearSelectedPlayers() {
    setState(() {
      ...
      print(
          "selected players length:::::::" + selectedPlayers.length.toString());
  ...
  Future<List<PlayersData_>?> fetchUpcomingMatches(
      String team1_Id, String team2_Id, String token) async {
    final String apiUrl =
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (upcomingMatch.status == 1) {
          if (upcomingMatch.playersData != null &&
              upcomingMatch.playersData != "") {
            for (int i = 0; i < upcomingMatch.playersData!.length; i++) {
              if (upcomingMatch.playersData![i].roleType == "WICKET KEEPER") {
                ...
                print(
                    'Wicket Keeper Added: ${upcomingMatch.playersData![i].isPlay}');
              }
              ...
              if (upcomingMatch.playersData![i].roleType == "ALL ROUNDER") {
                ...
                print(
                    'All Rounder Added: ${upcomingMatch.playersData![i].isPlay}');
              }
            ...
            SchedulerBinding.instance.addPostFrameCallback((_) {
              ...
              if (upcomingMatch.lineup == 1) {
                List<PlayersData_> Wk_not_InLineup =
                    widget.Wk_PlayersData.where((player) => player.isPlay == 0)
                        .toList();

                List<PlayersData_> Bat_not_InLineup =
                    widget.BatData_Player.where((player) => player.isPlay == 0)
                        .toList();

                List<PlayersData_> Ar_not_InLineup =
                    widget.ARData_Player.where((player) => player.isPlay == 0)
                        .toList();

                List<PlayersData_> Bowl_in_Lineup =
                    widget.BowlData_Player.where((player) => player.isPlay == 0)
                        .toList();

                bool hasUnannouncedPlayers = Wk_not_InLineup.isNotEmpty ||
                ...
                if (hasUnannouncedPlayers) {
                  showBottomSheetDialog(Wk_not_InLineup, Ar_not_InLineup,
                      Bat_not_InLineup, Bowl_in_Lineup);
                }
              }
            });
                ...
                TextButton(
                  ...
                  onPressed: () {
                    ...
                    Navigator.of(context)
                        .pop(); // To close the dialog and then the page
                  },
                ),
              ],
            );
                  ...
                  Row(
                    ...
                    children: [
                      Padding(
                        ...
                        child: Text('Players(${selectedPlayers.length})',
                    ...
                    children: [
                      ...
                      for (int i = 0; i < 11; i++) ...[
                        MiniContainer(
                          filled: i <
                              selectedPlayers
                                  .length, // Check if the MiniContainer should be filled
                        ),
                      ],
                      Padding(
                    ...
                    if (snapshot.connectionState == ConnectionState.waiting) {
                      ...
                      return RefreshIndicator(
                          ...
                          onRefresh: () async {
                            ...
                            setState(() {
                              Player_Data = fetchUpcomingMatches(
                                  widget.team1_id, widget.team2_id, _token);
                            });
                          },
                          child: Center(
                      ...
                      return ListView.builder(
                        ...
                        itemBuilder: (BuildContext context, int index) {
                         ...
                         // Wk_PlayersData.sort((a, b) => b.points.compareTo(a.points));
                          Wk_PlayersData.sort(
                              (a, b) => b.isPlay.compareTo(a.isPlay));
                          ...
                          PlayersData_ playerData = PlayersData_(
                              ...
                              isPlay: event.isPlay,
                              captionByuser: event.captionByuser,
                              VoiceCaptionByuser: event.VoiceCaptionByuser);



                          bool isPlayerMatchCondition =
                              widget.Wk_PlayersData.any((player) =>
                                  player.playerId == event.playerId);
                          print("team_Name" +
                              widget.text1 +
                              widget.team1_id +
                              ":::::" +
                              event.playingTeamId.toString() +
                              "::::::");
                          // print("data::::::::::::::::::3454::::"+Not_in_Lineup.length.toString());
                          bool isPlayerSelected =
                              selectedPlayers.contains(playerData);
                          return Padding(
                                  ...
                                  children: [
                                            ...
                                            color: event.playingTeamId != null
                                                ? widget.team1_id ==
                                                        event.playingTeamId
                                                            .toString()
                                                    ? Colors
                                                        .black // Set color for team1_id match
                                                    : Colors
                                                        .white // Set color for other team
                                                : Colors
                                                    .amberAccent, // Default color if playing_team_id is null
                                            borderRadius:
                                                ...
                                                color: event.playingTeamId != null
                                                    ? widget.team1_id ==
                                                            event.playingTeamId
                                                                .toString()
                                                        ? Colors
                                                            .white // Set text color for team1_id match
                                                        : Colors
                                                            .black // Set text color for other team
                                                    : Colors
                                                        .amberAccent, // Default text color if playing_team_id is null
                                                fontSize: 10,
                                    ...
                                    Column(
                                      ...
                                      children: [
                                        ...
                                        if (event.isPlay == 0 &&
                                            upcomingMatch.lineup == 0)
                                          Text(
                                            event.last_match_playing ==
                                                        1
                                                ? "Played last match"
                                            ...
                                            style: TextStyle(
                                              ...
                                              color: event.last_match_playing ==
                                                          1
                                                  ? Colors.green
                                                  : Colors.black,
                                            ),
                                        ...
                                        if (event.isPlay == 1 &&
                                            upcomingMatch.lineup == 1)
                                          Row(
                                            children: [
                                              Image(
                                                image: AssetImage(
                                                    ImageAssets.green_tic),
                                                height: 12,
                                                width: 12,
                                              ),
                                              Text(
                                            ...
                                            ? IconButton(
                                                ...
                                                onPressed: () {
                                                  setState(() {
                                                    ...
                                                    isMinusButtonVisibleList[
                                                        index] = false;
                                                    isPlusButtonVisibleList[
                                                        index] = true;
                                                    // }
                                                  });
                                                },
                                                          ...
                                                          Future.delayed(
                                                            ...
                                                            ScaffoldMessenger
                                                                ...
                                                                .showSnackBar(
                                                                    SnackBar(
                                                              content: Text(
                                                                  'You have reached the maximum player limit of $maxPlayerLimit.'),
                                                            ));
                                                          });
                                                            ...
                                                            Future.delayed(
                                                              ...
                                                              ScaffoldMessenger
                                                                  ...
                                                                  .showSnackBar(
                                                                      SnackBar(
                                                                content: Text(
                                                                    'Please select max 1 All Rounder.'),
                                                              ));
                                                            });
                                                          } else if (selectedPlayerBowl
                                                                  .length ==
                                                              0) {
                                                            ScaffoldMessenger
                                                                    .of(context)
                                                                .showSnackBar(
                                                                    SnackBar(
                                                              content: Text(
                                                                  'Please select max 1 Baller.'),
                                                            ));
                                                          } else if (selectedPlayerBat
                                                                  .length ==
                                                              0) {
                                                            ScaffoldMessenger
                                                                    .of(context)
                    ...
                    if (snapshot.connectionState == ConnectionState.waiting) {
                      ...
                      return ListView.builder(
                        ...
                        //  BatData_Player.sort((a, b) => b.points.compareTo(a.points));
                          ...
                          bool isPlayerMatchCondition =
                              widget.BatData_Player.any((player) =>
                                  player.playerId == event.playerId);
                                                          ...
                                                          if (selectedPlayerAR
                                                            ...
                                                            ScaffoldMessenger
                                                                ...
                                                                .showSnackBar(
                                                                    SnackBar(
                                                              content: Text(
                                                                  'Please select max 1 Wicket keeper.'),
                                                            ));
                                                          } else {
                                                            if (totalCredits >=
                                                                event.points) {
                    ...
                    if (snapshot.connectionState == ConnectionState.waiting) {
                      ...
                      return ListView.builder(
                       ...
                       //   ARData_Player.sort((a, b) => b.points.compareTo(a.points));
                          ...
                          bool isPlayerMatchCondition =
                              widget.ARData_Player.any((player) =>
                                  player.playerId == event.playerId);
                    ...
                    if (snapshot.connectionState == ConnectionState.waiting) {
                      ...
                      return ListView.builder(
                       ...
                       //   BowlData_Player.sort((a, b) => b.points.compareTo(a.points));
                          ...
                          bool isPlayerMatchCondition =
                              widget.BowlData_Player.any((player) =>
                                  player.playerId == event.playerId);
                                                          ...
                                                          if (selectedPlayerBat
                                                            ...
                                                            ScaffoldMessenger
                                                                ...
                                                                .showSnackBar(
                                                                    SnackBar(
                                                              content: Text(
                                                                  'Please select max 1 AllRounder.'),
                                                            ));
                                                          } else if (selectedPlayerWk
                                                                  .length ==
                                                              0) {
                      ...
                      ? () {
                          if (selectedPlayers.length != 11) {
                            Fluttertoast.showToast(
                                msg: "Please Select 11 Players");
                          } else if (selectedPlayerWk.length == 0) {
                            Fluttertoast.showToast(
                                msg:
                                    'At-least 1 Wicketkeeper should be selected');
                          } else if (selectedPlayerBowl.length == 0) {
                            Fluttertoast.showToast(
                                msg: 'At-least 1 Bowler should be selected');
                          } else if (selectedPlayerBat.length == 0) {
                            Fluttertoast.showToast(
                                msg: 'At-least 1 Batsman should be selected');
                          } else if (selectedPlayerAR.length == 0) {
                            Fluttertoast.showToast(
                                msg:
                                    'At-least 1 All rounder should be selected');
                          } else {
                            Navigator.push(
                                context,
                                MaterialPageRoute(
                                    builder: (context) => UpdateSaveTeam(
                                          batsmen: selectedPlayerBat,
                                          bowlers: selectedPlayerBowl,
                                          allrounders: selectedPlayerAR,
                                          wicketkeeper: selectedPlayerWk,
                                          ...
                                          captain_id: widget.captain_id,
                                          vice_captain_Id:
                                              widget.vice_captain_Id,

                                          selected_captain_Name:
                                              widget.selected_Captain_Name,
                                          selected_Vice_captain_Name:
                                              widget.selected_Vice_Captain_Name,
                                        )));
                          }
                        }
                      : null,
  ...
  void showBottomSheetDialog(
      List<PlayersData_> wk_InLineupData,
      List<PlayersData_> Ar_InLineupData,
      List<PlayersData_> Bat_InLineupData,
      List<PlayersData_> Bowl_InLineupData) {
